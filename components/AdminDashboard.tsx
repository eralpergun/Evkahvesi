  ...
  return (
    <div className="space-y-6 md:space-y-8 relative">
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-stone-100 overflow-hidden">
        <div className="px-4 md:px-8 py-4 md:py-6 border-b border-stone-50 flex justify-between items-center bg-stone-50/20">
          <div>
        ...
        {orders.length === 0 ? (
          ...
          <>
            ...
            <div className="block md:hidden divide-y divide-stone-100">
              {sortedOrders.map(order => (
                <div key={order.id} className={`p-4 transition-all ${order.status === 'COMPLETED' ? 'bg-stone-50/20 grayscale' : 'hover:bg-stone-50/30'}`}>
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-bold text-base text-stone-800">{order.guestName}</div>
                    ...
                    <div className="text-right">
                      <select 
                        value={order.status}
                        onChange={(e) => onUpdateStatus(order.id, e.target.value as Order['status'])}
                        className={`text-[10px] font-black px-2 py-1.5 rounded-lg border transition-all cursor-pointer outline-none ${
                          order.status === 'PENDING' ? 'bg-white text-stone-800 border-stone-200' :
                          order.status === 'PREPARING' ? 'bg-blue-50 text-blue-700 border-blue-100' :
                          'bg-green-50 text-green-700 border-green-100'
                        }`}
                  ...
                  <div className="bg-stone-50 rounded-xl p-3 mb-4 border border-stone-100">
                    <div className="flex justify-between items-center mb-2">
                      <div className="font-black text-stone-700 text-sm">{order.coffeeType}</div>
                      <div className="text-[10px] text-stone-500 font-bold uppercase tracking-tighter">{SIZE_LABELS[order.size]}</div>
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="flex-1 bg-stone-200 h-1.5 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full transition-all duration-700 ${order.status === 'COMPLETED' ? 'bg-stone-400' : 'bg-stone-800'}`} style={{ width: `${order.percentage}%` }}></div>
                      </div>
                      <span className="text-[10px] font-black text-stone-600">%{order.percentage}</span>
                      {order.milkLevel && (
                        <span className="px-2 py-1 bg-orange-100 text-orange-800 rounded-md text-[9px] font-bold uppercase tracking-wide border border-orange-200">
                          {order.milkLevel}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center justify-end gap-2">
                    {order.status !== 'COMPLETED' && (
                      <button 
                        onClick={() => onUpdateStatus(order.id, 'COMPLETED')}
                        className="flex-1 py-2.5 bg-stone-800 hover:bg-stone-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-md"
                      >
                        Servis Et
                      </button>
                    )}
                    <button 
                      onClick={() => onClearOrder(order.id)}
                      className="p-2.5 text-stone-400 hover:text-red-500 transition-all rounded-xl hover:bg-red-50 border border-stone-200 bg-white"
                    >
